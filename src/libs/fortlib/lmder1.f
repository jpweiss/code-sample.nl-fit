...
c
c Note from JPW:  This "relative error" appears to really be the d_x and
c     d_err of each iteration of the algorithm.  That is, once the
c     changes in the parameters or the error is smaller than tol, then
c     quit.
c
c
...
c
c
c     Machine constants for an SGI PowerChallenge R10k
c
c     For some reason, the compiler won't recognize hex constants, so
c     I'm leaving in those values here for historical reasons.  The
c     decimal equivalent is below.
c      data mcheps(1),mcheps(2) / x3CA00000, x00000000 /
c      data minmag(1),minmag(2) / x00100000, x00000000 /
c      data maxmag(1),maxmag(2) / x7FEFFFFF, xFFFFFFFF /
c
